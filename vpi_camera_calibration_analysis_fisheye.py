...
with open("config/distortion_correction.json", "w") as file:
    json.dump({"K": camMatrix.astype(float).tolist(), "dist_coeffs": coeffs.astype(float).tolist()}, file, indent=4)
...
for imgName in args.images:
    ...
    cv2.imwrite("config/calibrated_images/undistort_python{}_{:03d}.jpg".format(sys.version_info[0],idx), imgCorrected.cpu())
    idx += 1